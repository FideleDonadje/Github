...
OVERVIEW
-------------------------------------------------------------------------------------
We implemented KWIC using  in one class file using subroutines (methods in java) and shared data. Instead of having 4 seperate classes
we have one classes with several methods
...
We have one big classe Kwic that implements the 4 methods above and run the program. When running the main program,
each subroutine is called sequencially to produce the final result 
...
array of string 
	...
	All those methods directly modify the global variables and are called in order to get the final result